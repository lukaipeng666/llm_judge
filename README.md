...
```bash
python main.py \
  --api_urls http://localhost:8000/v1/chat/completions \
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `--api_urls` | str | - | 模型 API 地址（支持多个，空格分隔） |
| `--model` | str | Qwen/Qwen-1.8B-Chat | 模型名称 |
| `--data_file` | str | - | 测试数据文件路径（JSONL 格式） |
| `--data_id` | int | - | 数据库中的数据 ID（与 data_file 二选一） |
| `--scoring` | str | rouge | 评分函数名称 |
| `--scoring_module` | str | ./function_register/plugin.py | 自定义评分模块路径 |
| `--output` | str | evaluation_report.json | 报告输出路径 |
| `--max_workers` | int | 4 | 最大工作线程数 |
| `--badcase_threshold` | float | 1.0 | Badcase 判断阈值 |
| `--report_dir` | str | ./reports | 报告输出目录 |
| `--report_format` | str | json,txt,badcases | 报告格式（逗号分隔） |
| `--checkpoint_path` | str | - | 检查点文件路径 |
| `--checkpoint_interval` | int | 32 | 保存检查点的间隔 |
| `--resume` | flag | False | 从检查点继续运行 |
| `--role` | str | assistant | 测试角色 |
| `--timeout` | int | 600 | API 调用超时时间（秒） |
| `--max-tokens` | int | 16384 | 最大生成 token 数 |
| `--api_key` | str | sk-xxx... | API 密钥 |
| `--is_vllm` | flag | False | 是否使用 vLLM |
| `--temperature` | float | 0.0 | 生成温度 |
| `--top-p` | float | 1.0 | Top-p 采样参数 |
| `--test-mode` | flag | False | 测试模式（不实际调用 API） |
| `--sample-size` | int | 0 | 测试样本数量（0=全部） |
...
```bash
python main.py \
  --api_urls http://localhost:8000/generate/chat/completions \
...
python main.py \
  ...
  --checkpoint_path ./checkpoints/eval_checkpoint.json \
...
```bash
python main.py \
  --data_file ./data/custom_task.jsonl \
  --scoring my_custom_scorer \
  --scoring_module ./my_scoring_functions.py
```
...
```bash
python main.py \
  --data_file ./data/creative_writing.jsonl \
  --temperature 0.7 \
  --top-p 0.9 \
  --max-tokens 2048
```
...
```jsonl
{"instruction": "请回答以下问题", "input": "什么是人工智能？", "output": "人工智能是..."}
{"instruction": "翻译下列句子", "input": "Hello World", "output": "你好，世界"}
```
...
# 下载报告
GET /api/results/{task_id}/download?format=json
```

### 数据库 API (端口 16384)

提供数据持久化服务，详见 http://localhost:16384/docs
...
**A:** 
1. 增加 `--max_workers` 参数值
2. 使用多个 API 地址进行负载均衡
3. 调整 `--checkpoint_interval` 减少 I/O 开销

### Q: 评测任务中断后如何继续？

**A:** 使用 `--checkpoint_path` 指定检查点文件，中断后使用 `--resume` 参数继续：

```bash
python main.py --checkpoint_path ./checkpoint.json --resume
```
